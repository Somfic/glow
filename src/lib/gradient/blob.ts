export interface Blob {
	x: number;
	y: number;
	baseX: number; // Origin point for orbital motion
	baseY: number;
	radius: number;
	baseRadius: number; // Base size for pulsing
	color: string;
	speed: number; // Animation speed multiplier
	phaseX: number; // Phase offset for X movement
	phaseY: number; // Phase offset for Y movement
	orbitRadius: number; // How far from base position
}

export function createBlob(width: number, height: number, color: string): Blob {
...
export function updateBlob(blob: Blob, time: number, width: number, height: number): Blob {
...
export function drawBlob(ctx: CanvasRenderingContext2D, blob: Blob, opacity: number) {
	const gradient = ctx.createRadialGradient(blob.x, blob.y, 0, blob.x, blob.y, blob.radius);

	gradient.addColorStop(
		0,
		`${blob.color}${Math.floor(opacity * 255)
			.toString(16)
			.padStart(2, '0')}`
	);
