 ...
 * got — which matters when every check costs a page load and a build.
 */
export function checks(title) {
	...
	return {
		...
		/**
		 * Collect the page's console errors. The missing favicon and Vite's
		 * dev-server chatter are noise on every run; everything else is a finding.
		 */
		watch(page, ignore = [/favicon/, /404 \(Not Found\)/, /\[vite\]/]) {
			page.on("console", (m) => {
				const text = m.text();
